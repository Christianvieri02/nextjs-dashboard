...
export default function UserLayoutClient({
  ...
  useEffect(() => {
    try {
      ...
      setProfile({
        fullName: sess.fullName || sess.username || 'User',
        role: sess.role || 'User',
      });
    } catch (e) {
      console.error(e);
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3 pl-4 border-l border-gray-900">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-bold text-white leading-tight">{profile?.fullName || 'User'}</p>
              <p className="text-[9px] text-gray-500 font-medium">{profile?.role || 'User'} Access</p>
            </div>
